...
namespace MOE.Common.Business.SplitFail
{
    public class SplitFailChart
    {
        ...
        public SplitFailChart(DateTime graphStartDate, DateTime graphEndDate,
            MOE.Common.Business.CustomReport.Phase phase, string location, int firstSecondsofRed, 
            bool showFailLines, bool showAverageLines, bool showPercentFailLines, double? yAxisMax)
        {
            //double y = 0;
            ...
            extendedDirection = phase.ApproachDirection.ToString();
            ...
            //Set the chart title
            chart.Titles.Add(location + "Signal " + phase.SignalID.ToString() + " "
                + movementType + ": " + phase.PhaseNumber.ToString() +
            ...
            if (yAxisMax != null)
            {
                chartArea.AxisY.Maximum = (double)yAxisMax;
            }
            else
            {
            ...
            AddDataToChart(chart, phase, graphStartDate, graphEndDate, firstSecondsofRed, showFailLines, showAverageLines, showPercentFailLines);
        ...
        protected void AddDataToChart(Chart chart, MOE.Common.Business.CustomReport.Phase phase, DateTime startDate, DateTime endDate, int firstSecondsofRed, bool showFailLines, bool showAverageLines, bool showPercentFailLines)
        {
            ...
            var PhaseDetectors = signal.GetDetectorsForSignalThatSupportAMetricByPhaseNumber(12, phase.PhaseNumber);
            
            //get occupancy events for each lane for the approach.
            if (PhaseDetectors.Count() > 0)
            {
                foreach (Models.Detector row in PhaseDetectors)
                {

                    MOE.Common.Business.ControllerEventLogs TEMPdetectortable = new ControllerEventLogs(phase.SignalID, startDate, endDate, 
                ...
                if (Tables.Count > 0 )
                {
                    ...
                    foreach (MOE.Common.Business.CustomReport.Cycle c in phase.Cycles)
                    {
                        ...
                        if (c.CycleStart > startDate && c.CycleStart < endDate)
                        {

                            double gor = c.Activations.GreenOccupancy(c) * 100;
                            double ror = c.Activations.StartOfRedOccupancy(c, firstSecondsofRed) * 100;
                    ...
                    chart.Titles[0].Text += "\n Total split fails for the selected period = " + totalFails;
                    ...
                    //end of Cycle loop
                        ...
                        DateTime counterTime = startDate;
                        ...
                        do
                        {
                            ...
                            foreach (MOE.Common.Business.CustomReport.Cycle c in CycleBin)
                            {
                                ...
                                binTotalROR += c.Activations.StartOfRedOccupancy(c, firstSecondsofRed) * 100;



                            }
                        //);
                            if (showPercentFailLines)
                            {
                        ...
                        } while (counterTime < endDate.AddMinutes(15));
                ...
                AddPlanStrips(chart, phase, startDate, endDate, SplitFails);
            }
        }
