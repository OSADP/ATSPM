...
namespace MOE.Common.Business
{
    ...
    {    
        ...
        public RLMSignalPhase(DateTime startDate, DateTime endDate, int binSize, double srlvSeconds,
            int metricTypeID, MOE.Common.Models.Approach approach)
        {            
            ...
            {
                GetSignalPhaseData(startDate, endDate, showVolume, binSize);
            }
            else
            {
        ...
        private void GetSignalPhaseData(DateTime startDate, DateTime endDate, bool showVolume, int binSize)
        {
            ...
            var cycleEvents = controllerRepository.GetEventsByEventCodesParam(Approach.SignalID,
                startDate, endDate, new List<int>() { 1, 8, 9, 10, 11 }, Approach.ProtectedPhaseNumber);
