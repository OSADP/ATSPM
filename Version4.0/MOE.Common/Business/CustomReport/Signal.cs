...
namespace MOE.Common.Business.CustomReport
{
    public class Signal
    {
        ...
        {
            List<int> phases = SignalModel.GetPhasesForSignal();
            foreach (int p in phases)
            {
                List<Models.Detector> detectors = SignalModel.GetDetectorsForSignalByPhaseNumber(p);
                _Phases.Add(new Phase(detectors, Convert.ToInt32(p), _SignalID,
                    _StartDate, _EndDate, _EventCodes, startOfCycle));
            }
        }
